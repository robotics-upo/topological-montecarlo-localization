...
WallDetector::WallDetector(ros::NodeHandle &nh, ros::NodeHandle &pnh): PlaneDetectorROS(nh, pnh)
{
  ...
  wall_info_pub = nh.advertise<wall_detector::WallInfo>(wall_info_topic, 2);
}

WallDetector::WallDetector(double delta, double epsilon, double gamma, int theta, const std::string &cam, const Eigen::Affine3d& T): PlaneDetectorROS(delta, epsilon, gamma, theta), T(T)
{
