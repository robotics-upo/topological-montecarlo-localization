...
//   if (attributes.getLayer() == "POU_POSITION") {
...
void Test_CreationClass::addPolyline(const DL_PolylineData& data) {
    if (attributes.getLayer() == "TRAM") {
      
//       printf("POLYLINE \t");
//       printf("flags: %d \t m = %d\t n = %d\n", (int)data.flags, data.m, data.n);
    }
}
...
void Test_CreationClass::addVertex(const DL_VertexData& data) {
  // Isolated the trams
  if (attributes.getLayer() == "TRAM") {
//     printf("LAYER: %s \tVERTEX   (%6.3f, %6.3f, %6.3f) %6.3f\n",
//            attributes.getLayer().c_str(),
//            data.x, data.y, data.z,
//            data.bulge);
    curr_tram.push_back(data);
...
{
  printf("Ellipse: (%6.3f, %6.3f, %6.3f) m = (%6.3f %6.3f %6.3f)\n", 
         data.cx, data.cy, data.cz, data.mx, data.my, data.mz);
}

void Test_CreationClass::addBlock(const DL_BlockData&a)
{
//   printf("Block. Layer = %s\n", attributes.getLayer().c_str());
}

void Test_CreationClass::addInsert(const DL_InsertData& data)
{
  
  if (attributes.getLayer() == "POU") {
    pous.push_back(data);
//     printf("Insert POU: (%6.3f, %6.3f, %6.3f) ID = %d\n", 
//          data.ipx, data.ipy, data.ipz, attributes.getWidth());
  } else if (attributes.getLayer() == "ENTRONC") {
    entroncs.push_back(data);
//     printf("Insert ENTRONCS: (%6.3f, %6.3f, %6.3f)\n", 
//          data.ipx, data.ipy, data.ipz);
  }
  
//   printf("Insert. Layer = %s\n", attributes.getLayer().c_str());
}

void Test_CreationClass::endEntity()
{
  if (attributes.getLayer() == "TRAM") {
    trams.push_back(curr_tram);
    curr_tram.clear();
  }
}
